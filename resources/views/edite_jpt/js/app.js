...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ================================================================ */
  const scrollTargets = '.products-grid,.latest-products,.new-products,.brands,.cats';
  ...
  document.querySelectorAll(scrollTargets).forEach(el => {
    ...
    prev.addEventListener('click', () => el.scrollBy({ left: -250, behavior: 'smooth' }));
    next.addEventListener('click', () => el.scrollBy({ left:  250, behavior: 'smooth' }));
  });
  ...
  document.querySelectorAll('.mega-sidebar a').forEach(item => {
    item.addEventListener('mouseenter', () => {
      document.querySelectorAll('.mega-sidebar a').forEach(x => x.classList.remove('active'));
  ...
  (function () {
    ...
    let cartItems = [
      {
        id: 'p1', title: 'کابل شارژ سریع تایپ-سی انکر مدل PowerLine',
        cat: 'شارژر و کابل', price: 224000, oldPrice: 280000, qty: 1,
        img: 'https://images.unsplash.com/photo-1583863788434-e58a36330cf0?q=80&w=120&auto=format&fit=crop'
      },
      {
        id: 'p5', title: 'ساعت هوشمند سامسونگ گلکسی واچ ۶',
        cat: 'ساعت هوشمند', price: 3150000, oldPrice: 4500000, qty: 1,
    ...
    function renderWishlist() {
      ...
      // رندر HTML آیتم‌ها
      body.innerHTML = wishItems.map(item => `
        <div class="gm-item" data-id="${item.id}">
          <img class="gm-item__img" src="${item.img}" alt="${item.title}" loading="lazy">
          <div class="gm-item__info">
            <span class="gm-item__cat">${item.cat}</span>
      ...
      body.querySelectorAll('.gm-item__del').forEach(btn => {
        btn.addEventListener('click', () => {
          wishItems = wishItems.filter(i => i.id !== btn.dataset.id);
          // آپدیت ظاهر دکمه fav روی کارت محصول مربوطه (برگشت به حالت خط‌دار)
          const favBtn = document.querySelector(`.product-card[data-id="${btn.dataset.id}"] .product-card__fav`);
          if (favBtn) {
      ...
      body.querySelectorAll('.gm-add-cart-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          const favBtn = document.querySelector(`.product-card[data-id="${item.id}"] .product-card__fav`);
          if (favBtn) {
            favBtn.classList.remove('active');
            const favIcon = favBtn.querySelector('i');
            if (favIcon) favIcon.className = 'ti ti-heart';
    ...
    document.getElementById('cartClose')?.addEventListener('click', () => closeModal(cartModal, cartOverlay));
    cartOverlay?.addEventListener('click', () => closeModal(cartModal, cartOverlay));
    ...
    document.getElementById('wishlistClose')?.addEventListener('click', () => closeModal(wishModal, wishOverlay));
    wishOverlay?.addEventListener('click', () => closeModal(wishModal, wishOverlay));
...
/* Extracted from cart.html */
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      const suggested = [
        {
          id: 's1', title: 'پاوربانک انکر ۲۰۰۰۰ مگاپاور', cat: 'پاوربانک',
      ...
      function renderPageCart() {
        ...
        /* رندر آیتم‌ها */
        list.innerHTML = pageCartItems.map(item => {
          const discPct = item.oldPrice ? Math.round((1 - item.price / item.oldPrice) * 100) : 0;
          return `
        ...
        list.querySelectorAll('.cart-item__del').forEach(btn => {
          btn.addEventListener('click', () => {
            pageCartItems = pageCartItems.filter(i => i.id !== btn.dataset.id);
            renderPageCart();
          });
        });
        ...
        list.querySelectorAll('.qty-btn').forEach(btn => {
          btn.addEventListener('click', () => {
            ...
            if (btn.classList.contains('inc')) {
              ...
              pageCartItems = pageCartItems.filter(i => i.id !== item.id);
            }
            renderPageCart();
          });
        });
        ...
        /* محاسبه اعداد خلاصه */
        const origTotal = pageCartItems.reduce((s, i) => s + (i.oldPrice || i.price) * i.qty, 0);
        const discTotal = pageCartItems.reduce((s, i) => s + i.price * i.qty, 0);
        ...
        const progress = Math.min(100, Math.round(finalTotal / FREE_SHIP_THRESHOLD * 100));

        if (summaryOriginal) summaryOriginal.textContent = fmt(origTotal) + ' تومان';
        if (summaryDiscount) summaryDiscount.textContent = '−' + fmt(saved) + ' تومان';
        if (summaryTotal) summaryTotal.innerHTML = fmt(finalTotal) + ' <small>تومان</small>';

        /* پیام ارسال رایگان */
        if (remaining <= 0) {
          if (shippingMsg) shippingMsg.textContent = '🎉 ارسال این سفارش رایگان است!';
          if (shippingBar) shippingBar.style.width = '100%';
          if (document.getElementById('summaryShipping'))
            document.getElementById('summaryShipping').textContent = 'رایگان';
        } else {
          if (shippingMsg)
            shippingMsg.textContent = 'برای ارسال رایگان ' + fmt(remaining) + ' تومان دیگر خرید کنید';
      ...
      couponBtn?.addEventListener('click', () => {
        ...
        if (COUPONS[code]) {
          ...
          showCouponMsg('کد تخفیف ' + fa(COUPONS[code]) + '٪ با موفقیت اعمال شد!', 'green');
          renderPageCart();
        } else {
      ...
      function showCouponMsg(msg, type) {
        ...
        if (!el) {
          ...
          el.id = 'couponMsg';
          el.style.cssText = 'font-size:12px;font-weight:600;padding:6px 10px;border-radius:8px;margin-top:-6px;transition:opacity .3s';
          couponInput.parentElement.after(el);
        }
        const colors = { green: 'var(--c-green)', red: 'var(--c-red)', amber: 'var(--c-amber)' };
...
/* Extracted from login.html */
      ...
      function goToOtp(phone, context) {
        ...
        otpContext = context;
        document.getElementById("otpPhoneNumber").textContent =
          toFa(phoneClean);
